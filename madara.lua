            ...
            CreateSection: Function (
                ...
                AddTextbox: Function (
                    (table) Options [
                        ...
                        (EnumItem | nil) Font/TextFont = Enum.Font.Gotham
                ...
                AddDropdown: Function (
                    (table) Options [
                        ...
                        (string | function | table | nil) Filter = "StringToMatch" | Function (ValueToCheck) | Table [
                            ...
                            (any) [1] ... = "StringToMatch" | Enum.Font.Gotham/ValuesToMatch
                        ]
                ...
                AddSearchBox: Function (
                    (table) Options [
                        ...
                        (string | function | table | nil) Filter = "StringToMatch" | Function (ValueToCheck) | Table [
                            (boolean | nil) [0] = InverseBool
                            (string | any | nil) [1] ... = "StringToMatch" | Enum.Font.Gotham
                        ]
                        (string | function | table | nil) Method = "GetDescendants" | workspace.GetDescendants | Parameters [
                        	(string | function) [1/Method] = "GetFriendsOnline" | Player.GetFriendsOnline
                        	(any) [1/2/Args/Arguments] ... = 50
...
local library = {
	...
	colors = {
		main = Color3.fromRGB(219, 32, 46),
		background = Color3.fromRGB(20, 19, 21),
		outerBorder = Color3.fromRGB(9, 9, 11),
		innerBorder = Color3.fromRGB(96, 34, 44),
		topGradient = Color3.fromRGB(28, 26, 29),
		bottomGradient = Color3.fromRGB(17, 16, 18),
		sectionBackground = Color3.fromRGB(24, 22, 25),
...
local function resolveid(image, flag)
	if image then
		if type(image) == "string" then
			...
			if flag then
				local thing = (library.elements and library.elements[flag]) or (library.designerelements and library.designerelements[flag])
				...
				if getsynasset then
					...
					elseif (#image > 14) and (string.sub(image, 1, 14) == "synasseturl://") then
						local x, e = pcall(function()
							...
							local fold = isfolder("./madara.wtf")
							if fold then
							else
								makefolder("./madara.wtf")
							end
							fold = isfolder("./madara.wtf/Themes")
							if fold then
							else
								makefolder("./madara.wtf/Themes")
							end
							fold = isfolder("./madara.wtf/Themes/SynapseAssetsCache")
							if fold then
							else
								makefolder("./madara.wtf Themes/SynapseAssetsCache")
							end
							if not fold or not isfile("./madara.wtf/Themes/SynapseAssetsCache/" .. codename .. ".dat") then
								local res = game:HttpGet(string.sub(image, 15))
								if res ~= nil then
									writefile("./madara.wtf/Themes/SynapseAssetsCache/" .. codename .. ".dat", res)
								end
							end
							return getsynasset(readfile("./madara.wtf/Themes/SynapseAssetsCache/" .. codename .. ".dat"))
						end)
						if x and e ~= nil then
							return e
						end
		...
		if library.objects and (#library.objects > 0 or next(library.objects)) then
			...
			pcall(function()
				if library.Backdrop then
					library.Backdrop.Visible = (library_flags["__Designer.Background.UseBackgroundImage"] ~= false) and true
					library.Backdrop.Image = resolveid(library_flags["__Designer.Background.ImageAssetID"] or 10717609114, "__Designer.Background.ImageAssetID") or ""
					library.Backdrop.ImageColor3 = library_flags["__Designer.Background.ImageColor"] or Color3.new(1, 1, 1)
					library.Backdrop.ImageTransparency = (library_flags["__Designer.Background.ImageTransparency"] or 62) / 100
				end
			end)
		end
		MayGC -= 1
...
local function makeDraggable(topBarObject, object)
	...
	local function startRender()
		if renderConnection or not runService then
			return
		end
		renderConnection = runService.RenderStepped:Connect(function(dt)
			...
			local k = library.configuration.dragSmoothness or 16
			local alpha = 1 - math.exp(-k * dt)
			local nx = current.X.Offset + (targetPosition.X.Offset - current.X.Offset) * alpha
			local ny = current.Y.Offset + (targetPosition.Y.Offset - current.Y.Offset) * alpha
			object.Position = UDim2.new(targetPosition.X.Scale, nx, targetPosition.Y.Scale, ny)
			if not dragging then
				if (math.abs(targetPosition.X.Offset - nx) < 0.5) and (math.abs(targetPosition.Y.Offset - ny) < 0.5) then
	...
	library.signals[1 + #library.signals] = topBarObject.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
			...
			if library.configuration.smoothDragging then
				startRender()
			end
...
do
	...
	do
		...
		function library.Notify(self, NotificationData, ...)
			...
			local okCG, Slide = pcall(Instance.new, "CanvasGroup")
			if not okCG or not Slide then
				Slide = Instance.new("Frame")
				NotificationObj.NoGroupFade = true
			end
			Slide.Name = "Slide"
			Slide.BackgroundTransparency = 1
			Slide.BorderSizePixel = 0
			Slide.Size = UDim2.new(1, 0, 1, 0)
			Slide.Position = UDim2.new(0, 40, 0, 0)
			Slide.Parent = Notification
			Border.Parent = Slide
			Inner.Parent = Slide
			NotificationObj.Slide = Slide
			if NotificationObj.NoGroupFade then
				tweenService:Create(Slide, TweenInfo.new(0.32, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), { Position = UDim2.new(0, 0, 0, 0) }):Play()
			else
				Slide.GroupTransparency = 1
				tweenService:Create(Slide, TweenInfo.new(0.32, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), { GroupTransparency = 0, Position = UDim2.new(0, 0, 0, 0) }):Play()
			end
			...
			local function Destroy()
				...
				local SlideObj = NotificationObj.Slide
				...
				if Notification and Notification.Parent and SlideObj then
					local goal = { Position = UDim2.new(0, 40, 0, 0) }
					if not NotificationObj.NoGroupFade then
						goal.GroupTransparency = 1
					end
					tweenService:Create(SlideObj, TweenInfo.new(0.22, Enum.EasingStyle.Quart, Enum.EasingDirection.In), goal):Play()
					task.delay(0.24, finish)
				else
					finish()
				end
			end
...
function library:CreateWindow(options, ...)
	...
	if windowName and #windowName > 0 and library.WorkspaceName == "madara.wtf" then
		library.WorkspaceName = convertfilename(windowName, "madara.wtf")
	end
	local madaraGui = Instance_new("ScreenGui")
	library.MainScreenGui, MainScreenGui = madaraGui, madaraGui
	...
	madaraGui.Name = "     "
	madaraGui.Parent = library.gui_parent
	madaraGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
	madaraGui.DisplayOrder = 10
	madaraGui.ResetOnSpawn = false
	main.Name = "main"
	main.Parent = madaraGui
	...
	innerBackdrop.ScaleType = Enum.ScaleType.Crop
	innerBackdrop.Visible = (library_flags["__Designer.Background.UseBackgroundImage"] ~= false) and true
	innerBackdrop.ImageColor3 = library_flags["__Designer.Background.ImageColor"] or Color3.new(1, 1, 1)
	innerBackdrop.ImageTransparency = (library_flags["__Designer.Background.ImageTransparency"] or 62) / 100
	innerBackdrop.Image = resolveid(library_flags["__Designer.Background.ImageAssetID"] or 10717609114, "__Designer.Background.ImageAssetID") or ""
	...
	splitter.Font = Enum.Font.Gotham
	splitter.Text = "â€¢"
	...
	function windowFunctions:MoveTabSlider(tabObject)
		spawn(function()
			...
			tweenService:Create(tabSlider, TweenInfo.new(0.35, library.configuration.easingStyle, library.configuration.easingDirection), {
				Size = UDim2.fromOffset(tabObject.AbsoluteSize.X - 6, 2),
				Position = UDim2.fromOffset(tabObject.AbsolutePosition.X + 3, tabObject.AbsolutePosition.Y + tabObject.AbsoluteSize.Y + 1) - UDim2.fromOffset(main.AbsolutePosition.X, main.AbsolutePosition.Y)
			}):Play()
		end)
	end
	...
	function windowFunctions:CreateTab(options, ...)
		...
		if windowFunctions.tabCount == 1 then
			tabSlider.Size = UDim2.fromOffset(newTab.AbsoluteSize.X - 6, 2)
			tabSlider.Position = UDim2.fromOffset(newTab.AbsolutePosition.X + 3, newTab.AbsolutePosition.Y + newTab.AbsoluteSize.Y + 1) - UDim2.fromOffset(main.AbsolutePosition.X, main.AbsolutePosition.Y)
			tabSlider.Visible = true
			windowFunctions.selected.holder = newTabHolder
			windowFunctions.selected.button = newTab
		end
		...
		function tabFunctions:CreateSection(options, ...)
			...
			local sectionAccent = Instance_new("Frame")
			sectionAccent.Name = "sectionAccent"
			sectionAccent.Parent = newSectionBorder
			sectionAccent.BackgroundColor3 = library.colors.main
			colored[1 + #colored] = {sectionAccent, "BackgroundColor3", "main"}
			sectionAccent.BorderSizePixel = 0
			sectionAccent.Position = UDim2.new(0, 0, 0, 0)
			sectionAccent.Size = UDim2.new(0, 2, 1, 0)
			sectionAccent.ZIndex = 3
			local sectionGradient = Instance_new("UIGradient")
			sectionGradient.Name = "sectionGradient"
			sectionGradient.Parent = newSectionBorder
			sectionGradient.Rotation = 90
			sectionGradient.Color = ColorSequence.new(Color3.fromRGB(255, 255, 255), Color3.fromRGB(196, 196, 196))
			...
			sectionHeadline.Font = Enum.Font.Gotham
			...
			function sectionFunctions:AddToggle(options, ...)
				...
				toggleHeadline.Font = Enum.Font.Gotham
			...
			function sectionFunctions:AddDivider(options, ...)
				options = (options and type(options) == "string" and resolvevararg("Divider", options, ...)) or options or {}
				local dividerName = options.Name or options.Text
				local newDivider = Instance_new("Frame")
				newDivider.Name = "divider"
				newDivider.Parent = sectionHolder
				newDivider.BackgroundTransparency = 1
				newDivider.BorderSizePixel = 0
				newDivider.Size = UDim2.new(1, 0, 0, (dividerName and 16) or 10)
				local dividerLine = Instance_new("Frame")
				dividerLine.Name = "line"
				dividerLine.Parent = newDivider
				dividerLine.BorderSizePixel = 0
				dividerLine.BackgroundColor3 = library.colors.elementBorder
				colored[1 + #colored] = {dividerLine, "BackgroundColor3", "elementBorder"}
				dividerLine.AnchorPoint = Vector2.new(0, 0.5)
				dividerLine.Position = UDim2.new(0, 8, 0.5, 0)
				dividerLine.Size = UDim2.new(1, -16, 0, 1)
				local dividerLabel
				if dividerName then
					dividerLabel = Instance_new("TextLabel")
					dividerLabel.Name = "label"
					dividerLabel.Parent = newDivider
					dividerLabel.BackgroundColor3 = library.colors.sectionBackground
					colored[1 + #colored] = {dividerLabel, "BackgroundColor3", "sectionBackground"}
					dividerLabel.BorderSizePixel = 0
					dividerLabel.Font = Enum.Font.Gotham
					dividerLabel.Text = " " .. tostring(dividerName) .. " "
					dividerLabel.TextSize = 13
					dividerLabel.TextColor3 = library.colors.section
					colored[1 + #colored] = {dividerLabel, "TextColor3", "section"}
					dividerLabel.AnchorPoint = Vector2.new(0.5, 0.5)
					dividerLabel.Position = UDim2.fromScale(0.5, 0.5)
					dividerLabel.Size = UDim2.fromOffset(textToSize(dividerLabel).X + 6, 14)
				...
				local objectdata = {
					...
					SetText = function(t, str)
						...
						if dividerLabel then
							dividerLabel.Text = " " .. tostring(str) .. " "
							dividerLabel.Size = UDim2.fromOffset(textToSize(dividerLabel).X + 6, 14)
						end
						return str
					end
			...
			sectionFunctions.CreateDivider = sectionFunctions.AddDivider
			sectionFunctions.NewDivider = sectionFunctions.AddDivider
			sectionFunctions.Divider = sectionFunctions.AddDivider
			function sectionFunctions:AddTextbox(options, ...)
				...
				textboxHeadline.Font = Enum.Font.Gotham
			...
			function sectionFunctions:AddKeybind(options, ...)
				...
				keybindHeadline.Font = Enum.Font.Gotham
			...
			function sectionFunctions:AddLabel(options, ...)
				...
				labelHeadline.Font = Enum.Font.Gotham
			...
			function sectionFunctions:AddSlider(options, ...)
				...
				sliderHeadline.Font = Enum.Font.Gotham
			...
			function sectionFunctions:AddSearchBox(options, ...)
				...
				dropdownHeadline.Font = Enum.Font.Gotham
			...
			if isfolder and makefolder and listfiles and readfile and writefile then
				function sectionFunctions:AddPersistence(options, ...)
					...
					local realDropdownHolderList = Instance_new("UIListLayout")
					local dropdownEnabled = false
					if not isfolder("./madara.wtf") then
						makefolder("./madara.wtf")
					end
					local common_string = "./madara.wtf/" .. tostring(custom_workspace or library.WorkspaceName)
					local function resolvelist(nofold)
						if custom_workspace ~= options.Workspace then
							custom_workspace = options.Workspace
							common_string = "./madara.wtf/" .. tostring(custom_workspace or library.WorkspaceName)
						end
			...
			function sectionFunctions:AddColorpicker(options, ...)
				...
				colorPickerHeadline.Font = Enum.Font.Gotham
				...
				hexInputBox.Font = Enum.Font.Gotham
	...
	function windowFunctions:CreateDesigner(options, ...)
		...
		if options.Credit ~= false then
			daaata[1 + #daaata] = {"AddLabel", "__Designer.Label.Creator", detailssection, {
				Text = "madara.wtf  |  base by Pepsi#5229 "
			}}
...
do
	...
	local function resolveTarget(target)
		if type(target) == "table" then
			return target.Instance or target.ButtonFrame or target.Frame
		end
		return target
	end

	local watermark, wmLabel, wmConn
	function library:SetWatermark(text)
		if rawequal(self, library) then else text = self end
		text = tostring(text or "madara.wtf")
		library.watermarkText = text
		if not library.MainScreenGui then return end
		if not watermark then
			watermark = Instance_new("Frame")
			watermark.Name = "watermark"
			watermark.Parent = library.MainScreenGui
			watermark.BackgroundColor3 = library.colors.background
			colored[1 + #colored] = {watermark, "BackgroundColor3", "background"}
			watermark.BorderColor3 = library.colors.outerBorder
			colored[1 + #colored] = {watermark, "BorderColor3", "outerBorder"}
			watermark.Position = UDim2.fromOffset(16, 16)
			watermark.Size = UDim2.fromOffset(220, 24)
			watermark.ZIndex = 5000
			local wmAccent = Instance_new("Frame")
			wmAccent.Name = "accent"
			wmAccent.Parent = watermark
			wmAccent.BackgroundColor3 = library.colors.main
			colored[1 + #colored] = {wmAccent, "BackgroundColor3", "main"}
			wmAccent.BorderSizePixel = 0
			wmAccent.Position = UDim2.new(0, 0, 0, 0)
			wmAccent.Size = UDim2.new(1, 0, 0, 2)
			wmAccent.ZIndex = 5001
			wmLabel = Instance_new("TextLabel")
			wmLabel.Name = "text"
			wmLabel.Parent = watermark
			wmLabel.BackgroundTransparency = 1
			wmLabel.Position = UDim2.new(0, 10, 0, 1)
			wmLabel.Size = UDim2.new(1, -20, 1, -1)
			wmLabel.Font = Enum.Font.GothamBold
			wmLabel.TextSize = 14
			wmLabel.TextXAlignment = Enum.TextXAlignment.Left
			wmLabel.TextColor3 = library.colors.tabText
			colored[1 + #colored] = {wmLabel, "TextColor3", "tabText"}
			wmLabel.ZIndex = 5001
			local frames, lastClock = 0, os.clock()
			wmConn = runService.RenderStepped:Connect(function()
				frames = frames + 1
				local now = os.clock()
				if now - lastClock >= 0.5 then
					local fps = math.floor((frames / (now - lastClock)) + 0.5)
					frames, lastClock = 0, now
					if wmLabel and watermark and watermark.Parent then
						wmLabel.Text = (library.watermarkText or "madara.wtf") .. "   |   fps: " .. fps
						watermark.Size = UDim2.fromOffset(math.max(150, textToSize(wmLabel).X + 24), 24)
					end
				end
			end)
			if library.signals then
				library.signals[1 + #library.signals] = wmConn
			end
	...
	library.Watermark = library.SetWatermark
	function library:SetWatermarkVisibility(state)
		if rawequal(self, library) then else state = self end
		if watermark then
	...
	local function ensureTooltip()
		if tip or not library.MainScreenGui then return end
		tip = Instance_new("Frame")
		tip.Name = "tooltip"
		tip.Parent = library.MainScreenGui
		tip.BackgroundColor3 = library.colors.background
		colored[1 + #colored] = {tip, "BackgroundColor3", "background"}
		tip.BorderColor3 = library.colors.outerBorder
		colored[1 + #colored] = {tip, "BorderColor3", "outerBorder"}
		tip.Visible = false
		tip.ZIndex = 6000
		tip.Size = UDim2.fromOffset(120, 22)
		local tipAccent = Instance_new("Frame")
		tipAccent.Name = "accent"
		tipAccent.Parent = tip
		tipAccent.BackgroundColor3 = library.colors.main
		colored[1 + #colored] = {tipAccent, "BackgroundColor3", "main"}
		tipAccent.BorderSizePixel = 0
		tipAccent.Size = UDim2.new(0, 2, 1, 0)
		tipAccent.ZIndex = 6001
		tipLabel = Instance_new("TextLabel")
		tipLabel.Name = "text"
		tipLabel.Parent = tip
		...
		tipLabel.TextColor3 = library.colors.tabText
		colored[1 + #colored] = {tipLabel, "TextColor3", "tabText"}
		tipLabel.ZIndex = 6001
	end
	function library:AddTooltip(target, text)
		if rawequal(self, library) then else target, text = self, target end
		...
		local function move()
			...
			tip.Position = UDim2.fromOffset(mx + 16, my + 8)
		end
		local function show()
			tipLabel.Text = text
			tip.Size = UDim2.fromOffset(textToSize(tipLabel).X + 16, 22)
			move()
			tip.Visible = true
		end
		local function hide()
		...
		local c2 = target.MouseMoved:Connect(move)
		local c3 = target.MouseLeave:Connect(hide)
		if library.signals then
			library.signals[1 + #library.signals] = c1
			library.signals[1 + #library.signals] = c2
			library.signals[1 + #library.signals] = c3
		end
	end
	library.Tooltip = library.AddTooltip

	function library:AddContextMenu(target, optionsList)
		if rawequal(self, library) then else target, optionsList = self, target end
		target = resolveTarget(target)
		if not target or not library.MainScreenGui then return end
		optionsList = optionsList or {}
		local menu = Instance_new("Frame")
		menu.Name = "contextMenu"
		menu.Parent = library.MainScreenGui
		menu.BackgroundColor3 = library.colors.background
		colored[1 + #colored] = {menu, "BackgroundColor3", "background"}
		menu.BorderColor3 = library.colors.outerBorder
		colored[1 + #colored] = {menu, "BorderColor3", "outerBorder"}
		menu.Visible = false
		menu.ZIndex = 7000
		local menuAccent = Instance_new("Frame")
		menuAccent.Name = "accent"
		menuAccent.Parent = menu
		menuAccent.BackgroundColor3 = library.colors.main
		colored[1 + #colored] = {menuAccent, "BackgroundColor3", "main"}
		menuAccent.BorderSizePixel = 0
		menuAccent.Size = UDim2.new(1, 0, 0, 2)
		menuAccent.ZIndex = 7002
		local menuList = Instance_new("UIListLayout")
		menuList.Parent = menu
		menuList.SortOrder = Enum.SortOrder.LayoutOrder
		local menuPad = Instance_new("UIPadding")
		menuPad.Parent = menu
		menuPad.PaddingTop = UDim.new(0, 3)
		local width = 130
		for i, opt in ipairs(optionsList) do
			local row = Instance_new("TextButton")
			row.Name = "option"
			row.Parent = menu
			row.LayoutOrder = i
			row.BackgroundColor3 = library.colors.sectionBackground
			colored[1 + #colored] = {row, "BackgroundColor3", "sectionBackground"}
			row.BorderSizePixel = 0
			row.AutoButtonColor = false
			row.Size = UDim2.new(1, 0, 0, 22)
			row.Font = Enum.Font.Gotham
			row.Text = "  " .. tostring(opt.Name or opt.Text or "Option")
			row.TextSize = 13
			row.TextXAlignment = Enum.TextXAlignment.Left
			row.TextColor3 = library.colors.elementText
			colored[1 + #colored] = {row, "TextColor3", "elementText"}
			row.ZIndex = 7001
			width = math.max(width, textToSize(row).X + 24)
			local c_enter = row.MouseEnter:Connect(function()
				tweenService:Create(row, TweenInfo.new(0.2), { BackgroundColor3 = darkenColor(library.colors.main, 1.6) }):Play()
				row.TextColor3 = Color3.new(1, 1, 1)
			end)
			local c_leave = row.MouseLeave:Connect(function()
				tweenService:Create(row, TweenInfo.new(0.2), { BackgroundColor3 = library.colors.sectionBackground }):Play()
				row.TextColor3 = library.colors.elementText
			end)
			local c_click = row.MouseButton1Click:Connect(function()
				menu.Visible = false
			...
			if library.signals then
				library.signals[1 + #library.signals] = c_enter
				library.signals[1 + #library.signals] = c_leave
				library.signals[1 + #library.signals] = c_click
			end
		end
		menu.Size = UDim2.fromOffset(width, (#optionsList * 22) + 6)
		local c_open = target.InputBegan:Connect(function(input)
		...
		local c_close = userInputService.InputBegan:Connect(function(input)
			...
			if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.MouseButton2 then
				...
				if not inside and input.UserInputType == Enum.UserInputType.MouseButton1 then
					menu.Visible = false
				end
			end
		end)
		if library.signals then
			library.signals[1 + #library.signals] = c_open
			library.signals[1 + #library.signals] = c_close
		end
		return {
			Menu = menu,
			Show = function() menu.Visible = true end,
			Hide = function() menu.Visible = false end,
			Remove = function() menu.Parent = nil end
		}
	end
	library.ContextMenu = library.AddContextMenu

	local cursorActive = false
	local cursorConn, cursorTri, cursorOutline
	local function removeCursor()
		if cursorConn then
			cursorConn:Disconnect()
			cursorConn = nil
		end
		pcall(function() if cursorTri then cursorTri:Remove() end end)
		pcall(function() if cursorOutline then cursorOutline:Remove() end end)
		cursorTri, cursorOutline = nil, nil
		pcall(function() userInputService.MouseIconEnabled = true end)
	end
	function library:SetCustomCursor(enabled)
		if rawequal(self, library) then else enabled = self end
		enabled = (enabled == nil) or (enabled == true) or (enabled == 1)
		library.configuration.customCursor = enabled
		if not enabled then
			cursorActive = false
			removeCursor()
			return false
		...
		if not Drawing then
			warn("[madara.wtf] custom cursor requires a Drawing API (not available in this environment)")
			library.configuration.customCursor = false
			return false
		end
		local ok = pcall(function()
			cursorTri = Drawing.new("Triangle")
			cursorTri.Thickness = 1
			cursorTri.Filled = true
			cursorTri.Visible = false
		...
		cursorConn = runService.RenderStepped:Connect(function()
			...
			local visible = library.MainScreenGui ~= nil
				and library.MainScreenGui.Parent ~= nil
				and (library.MainFrame == nil or library.MainFrame.Visible)
			if not visible then
				pcall(function() userInputService.MouseIconEnabled = true end)
				cursorTri.Visible = false
				cursorOutline.Visible = false
			...
			local m = userInputService:GetMouseLocation()
			local x, y = m.X, m.Y
			cursorTri.Color = library.colors.main
			cursorTri.PointA = Vector2.new(x, y)
			cursorTri.PointB = Vector2.new(x + 16, y + 6)
			cursorTri.PointC = Vector2.new(x + 6, y + 16)
			cursorTri.Visible = true
			cursorOutline.PointA = cursorTri.PointA
			cursorOutline.PointB = cursorTri.PointB
			cursorOutline.PointC = cursorTri.PointC
			cursorOutline.Visible = true
		end)
		if library.signals then
			library.signals[1 + #library.signals] = cursorConn
		end
		return true
	end
	library.CustomCursor = library.SetCustomCursor
end
...
do
	local themePresets = {
		...
		Ocean = {
			main = Color3.fromRGB(32, 140, 219),
			innerBorder = Color3.fromRGB(34, 72, 96),
			background = Color3.fromRGB(17, 20, 24),
			outerBorder = Color3.fromRGB(8, 10, 12),
			topGradient = Color3.fromRGB(22, 26, 30),
			bottomGradient = Color3.fromRGB(14, 17, 20),
			sectionBackground = Color3.fromRGB(20, 24, 28)
		},
		Emerald = {
			main = Color3.fromRGB(46, 204, 113),
			innerBorder = Color3.fromRGB(30, 80, 55),
			background = Color3.fromRGB(18, 22, 20),
			outerBorder = Color3.fromRGB(8, 11, 9),
			topGradient = Color3.fromRGB(24, 30, 26),
			bottomGradient = Color3.fromRGB(15, 19, 16),
			sectionBackground = Color3.fromRGB(21, 27, 23)
		},
		Mono = {
			main = Color3.fromRGB(200, 200, 205),
			innerBorder = Color3.fromRGB(70, 70, 75),
			background = Color3.fromRGB(20, 20, 22),
			outerBorder = Color3.fromRGB(9, 9, 10),
			topGradient = Color3.fromRGB(28, 28, 30),
			bottomGradient = Color3.fromRGB(17, 17, 19),
			sectionBackground = Color3.fromRGB(24, 24, 26)
		}
	}
	library.ThemePresets = themePresets

	local function applyColors(map, tweenTime)
		local mt = getmetatable(library.colors)
		local raw = mt and mt.__index
		if raw then
			for key, color in next, map do
				raw[key] = color
			end
			if library.subs.updatecolors then
				library.subs.updatecolors(tweenTime)
			end
		else
			for key, color in next, map do
				library.colors[key] = color
			end
		end
	end

	function library:SetTheme(preset)
		if rawequal(self, library) then else preset = self end
		local map = (type(preset) == "table" and preset) or themePresets[tostring(preset)]
		if not map then
			return false
		end
		library.activeTheme = (type(preset) == "string" and preset) or library.activeTheme
		applyColors(map, 0.25)
		return true
	end
	library.ApplyTheme = library.SetTheme

	function library:SetAccent(color)
		if rawequal(self, library) then else color = self end
		if typeof(color) ~= "Color3" then
			return false
		end
		applyColors({
			main = color,
			innerBorder = color:Lerp(Color3.new(0, 0, 0), 0.55)
		}, 0.2)
		return true
	end
	library.SetAccentColor = library.SetAccent

	local configFolder = "madara.wtf"
	...
	local function configPath(name)
		return configDir() .. "/" .. sanitize(name) .. ".json"
	end
	local function autoloadPath()
		return configDir() .. "/autoload.txt"
	end

	function library:SetConfigFolder(folder)
		if rawequal(self, library) then else folder = self end
		configFolder = sanitize(folder)
		if #configFolder == 0 then
			configFolder = "madara.wtf"
		end
		return configFolder
	end
	...
	function library:GetConfigs()
		...
		local out = {}
		for _, path in next, listfiles(configDir()) do
			local file = tostring(path):match("([^/\\]+)%.json$")
			if file then
				out[1 + #out] = file
			end
		end
	...
	local function serializeFlags()
		local saving = {}
		for cflag, data in next, (library.elements or {}) do
			if data and data.Type ~= "Persistence" and data.Type ~= "Button" and string.sub(cflag, 1, 11) ~= "__Designer." and not (data.Options and data.Options.ExcludeFromConfig) then
				local value = library.flags[cflag]
				if value ~= nil then
					local ok, jval = library.JSONEncode(value)
					if not ok or jval == "null" then
						if typeof(value) == "Color3" then
							value = (library.rainbowflags[cflag] and "rainbow") or library.subs.Color3ToHex(value)
						else
							value = tostring(value)
						end
					end
	...
	local function applyLoaded(jcontent)
		for cflag, val in next, jcontent do
			if type(val) == "string" and #val > 7 and #val < 64 and string.sub(val, 1, 5) == "Enum." then
				local dot = string.find(val, ".", 6, true)
				if dot then
					local enum = Enum[string.sub(val, 6, dot - 1)]
					enum = enum and enum[string.sub(val, dot + 1)]
					if enum then
						val = enum
					end
				end
			end
			local data = library.elements[cflag]
			if data and data.Type ~= "Persistence" then
				if data.Set then
					pcall(data.Set, data, val)
				else
					library.flags[cflag] = val
				end
			end
	...
	function library:SaveConfig(name)
		if rawequal(self, library) then else name = self end
		if not ensureFolders() then
			return false, "No file access in this environment"
		end
		name = sanitize(name)
		if #name == 0 then
			return false, "Invalid config name"
		end
		local ok, content = library.JSONEncode(serializeFlags())
		if not ok or not content then
			return false, "Failed to encode config"
		end
		writefile(configPath(name), content)
		return true, name
	end
	...
	function library:LoadConfig(name)
		...
		local ok, jcontent = library.JSONDecode(content)
		if not ok or type(jcontent) ~= "table" then
			return false, "Failed to decode config"
		end
		applyLoaded(jcontent)
		return true, name
	end
	...
	function library:SetAutoloadConfig(name)
		...
		writefile(autoloadPath(), sanitize(name))
		return true
	end
	...
	function library:AddConfigManager(section, opts)
		if rawequal(self, library) then else section, opts = self, section end
		opts = opts or {}
		local initialList = library:GetConfigs()
		if #initialList == 0 then
			initialList = { "[none]" }
		end
		local nameBox = section:AddTextbox({ Name = "Config Name", Placeholder = "config name...", ExcludeFromConfig = true, Callback = function() end })
		local listDrop = section:AddDropdown({ Name = "Configs", List = initialList, EmptyValue = "[none]", ExcludeFromConfig = true, Callback = function() end })
		local autoLabel = section:AddLabel({ Name = "Autoload: " .. (library:GetAutoloadConfig() or "none") })
		local function refresh()
			local cfgs = library:GetConfigs()
			if #cfgs == 0 then
				cfgs = { "[none]" }
			end
			listDrop:UpdateList(cfgs)
			autoLabel.Set("Autoload: " .. (library:GetAutoloadConfig() or "none"))
		end
		local function chosen()
			local typed = nameBox.Get()
			if typed and #tostring(typed) > 0 then
				return typed
			end
			local sel = listDrop.Get()
			if sel == "[none]" then
				return nil
			end
			return sel
		...
		section:AddButton({
			{
				Name = "Save",
				Callback = function()
					local ok, msg = library:SaveConfig(chosen())
					library:Notify({ Text = ok and ("Saved config: " .. tostring(msg)) or ("Save failed: " .. tostring(msg)), Time = 3 })
					if ok then
						refresh()
					end
				end
			},
			{
				Name = "Load",
				Callback = function()
					local ok, msg = library:LoadConfig(chosen())
					library:Notify({ Text = ok and ("Loaded config: " .. tostring(msg)) or ("Load failed: " .. tostring(msg)), Time = 3 })
				end
			}
		})
		section:AddButton({
			{
				...
				Callback = function()
					...
					if library:DeleteConfig(name) then
						library:Notify({ Text = "Deleted config: " .. tostring(name), Time = 3 })
						refresh()
					else
						library:Notify({ Text = "Config not found", Time = 3 })
					end
				end
			},
			{
		...
		section:AddButton({ Name = "Set as Autoload", Callback = function()
			local name = chosen()
			if name and #tostring(name) > 0 and library:SetAutoloadConfig(name) then
				library:Notify({ Text = "Autoload set to: " .. tostring(name), Time = 3 })
				refresh()
			end
		end })
		return {
			...
			NameBox = nameBox,
			List = listDrop,
			AutoloadLabel = autoLabel
		}
	end

	local keyList, keyListContent
	local keyPickers = {}
	local function shortKeyName(key)
		...
		if typeof(key) == "EnumItem" then
			return keyHandler.allowedKeys[key] or key.Name
		end
		return tostring(key)
	end
	local function refreshKeyList()
		...
		local shown, widest = 0, 150
		for _, kp in next, keyPickers do
			if kp.row.Visible then
				shown = shown + 1
				local w = textToSize(kp.row).X + 20
				if w > widest then
					widest = w
				end
			end
		end
		keyList.Visible = shown > 0
		keyList.Size = UDim2.fromOffset(widest, 24 + (shown * 16) + ((shown > 0) and 6 or 0))
	end
	local function ensureKeyList()
		if keyList or not library.MainScreenGui then
			return keyList
		end
		keyList = Instance_new("Frame")
		keyList.Name = "keybinds"
		keyList.Parent = library.MainScreenGui
		keyList.AnchorPoint = Vector2.new(1, 0)
		keyList.Position = UDim2.new(1, -16, 0, 16)
		keyList.Size = UDim2.fromOffset(160, 24)
		keyList.BackgroundColor3 = library.colors.background
		colored[1 + #colored] = { keyList, "BackgroundColor3", "background" }
		keyList.BorderColor3 = library.colors.outerBorder
		colored[1 + #colored] = { keyList, "BorderColor3", "outerBorder" }
		keyList.Visible = false
		keyList.ZIndex = 5000
		local accent = Instance_new("Frame")
		accent.Name = "accent"
		accent.Parent = keyList
		accent.BackgroundColor3 = library.colors.main
		colored[1 + #colored] = { accent, "BackgroundColor3", "main" }
		...
		title.Size = UDim2.new(1, -16, 0, 15)
		title.Font = Enum.Font.GothamBold
		title.TextSize = 13
		title.TextXAlignment = Enum.TextXAlignment.Left
		title.Text = "Keybinds"
		title.TextColor3 = library.colors.tabText
		colored[1 + #colored] = { title, "TextColor3", "tabText" }
		title.ZIndex = 5001
		keyListContent = Instance_new("Frame")
		keyListContent.Name = "content"
		keyListContent.Parent = keyList
		keyListContent.BackgroundTransparency = 1
		keyListContent.Position = UDim2.new(0, 8, 0, 22)
		keyListContent.Size = UDim2.new(1, -16, 1, -26)
		keyListContent.ZIndex = 5001
		local layout = Instance_new("UIListLayout")
		layout.Parent = keyListContent
		layout.SortOrder = Enum.SortOrder.LayoutOrder
		layout.Padding = UDim.new(0, 2)
		makeDraggable(keyList, keyList)
		return keyList
	end

	function library:AddKeyPicker(opts)
		if rawequal(self, library) then else opts = self end
		opts = opts or {}
		ensureKeyList()
		local name = tostring(opts.Name or opts.Text or "Keybind")
		local mode = tostring(opts.Mode or "Toggle")
		local callback = opts.Callback
		local key = opts.Key or opts.Default
		local flag = opts.Flag and tostring(opts.Flag)
		local state = false
		local binding = false
		local kp = {}
		...
		local function render()
			local suffix = (mode ~= "Toggle") and (" " .. mode) or ""
			if binding then
				row.Text = name .. "  [ ... ]"
			else
				row.Text = name .. "  [" .. shortKeyName(key) .. suffix .. "]"
			end
			row.TextColor3 = state and library.colors.main or library.colors.elementText
			refreshKeyList()
		end
		...
		kp.SetMode = function(a, b)
			local m = (a == kp) and b or a
			mode = tostring(m or mode)
			if mode == "Always" then
				setState(true)
		...
		library.signals[1 + #library.signals] = userInputService.InputBegan:Connect(function(input, gpe)
			if binding then
				if input.UserInputType == Enum.UserInputType.Keyboard then
		...
		library.signals[1 + #library.signals] = row.MouseButton1Click:Connect(function()
			binding = true
			render()
		end)
		...
		if flag then
			...
			elements[flag] = {
				...
				Set = function(a, b)
					kp.SetKey((a == elements[flag]) and b or a)
				end,
				RawSet = function(a, b)
					kp.SetKey((a == elements[flag]) and b or a)
				end
		...
		keyPickers[1 + #keyPickers] = kp
		render()
		return kp
	end
	library.KeyPicker = library.AddKeyPicker

	local baseUnload = library.unload
	...
	library.unload = unloadEverything
	library.Unload = unloadEverything
end
